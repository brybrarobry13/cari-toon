			...
			if($_FILES['product_image']['name']!='')
			{
					...
					
				{
					...
					{	
						...
						$sql_insert="INSERT INTO `toon_main_category`(`mcat_name`,`mcat_image`,`mcat_description`,`mcat_display`)VALUES ('$product_cat','$photoName','$product_des','$product_display')";	
						mysql_query($sql_insert);
						$name=mysql_insert_id();																			  						 				move_uploaded_file($_FILES['product_image']['tmp_name'],DIR_EZPRINTS_CAT_IMAGES.$name.'_'.$photoName);
						//move_uploaded_file($_FILES['product_image']['tmp_name'],EZ_maincat_imgs.$name.'_'.$photoName);
						$newname=$name.'_'.$photoName;
						mysql_query("UPDATE `toon_main_category` SET `mcat_image`='$newname'WHERE `mcat_id`='$name'");
...
<form action="" method="post"  onsubmit="return verify();" enctype="multipart/form-data">
    ...
    <tr>
      ...
      <td align="center"><table cellpadding="0" cellspacing="0" width="100%" class="table_border">
          <tr class="table_titlebar">
            <td class="main_heading" height="30" colspan="4">Add EZ Main Category</td>
          </tr>
          <tr>
		  ...
		  <tr>
			...
			<div id="product_image_err" style="display:none;color:#FF0000" >Enter EZ Main Category Image</div>
